...
from app.models import db, Task, User, Invitation, Participant
...
def view_received_invitations():
    try:
        ...
        invitations_data = [
            invitation.to_dict_with_task_details() for invitation in invitations
        ]
...
def send_invitations():
    ...
    if not task_id or not invitee_ids or not isinstance(invitee_ids, list):
    ...
    if task.owner_id != current_user.id:
        return jsonify({"error": "Unauthorized to invite users to this task."}), 403
    ...
    for invitee_id in invitee_ids:
        if Invitation.check_time_conflict_for_invitee(
            invitee_id, task.start_time, task.end_time
        ):
            return jsonify(
        ...
        existing_invitation = Invitation.query.filter_by(
            task_id=task_id, invitee_id=invitee_id
        ).first()
        ...
        return jsonify(
            {
                ...
                "invitations": [
                    inv.to_dict_with_task_details() for inv in invitations
                ],
            }
        ), 200
...
def respond_to_invitation():
    ...
    if response == "accepted":
        ...
        existing_participant = Participant.query.filter_by(
            task_id=invitation.task_id, user_id=current_user.id
        ).first()
